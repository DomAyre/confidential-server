...
def test_requesting_file_in_config():
    ...
    response = server.post('/fetch/readme.md', json={'wrapping_key': format_public_key(generate_key_pair()[1])})
    assert response.status_code == 200


def test_posting_file_which_doesnt_exist():
    server = get_test_client("examples/config/single_file_single_policy.yml")
    response = server.post('/fetch/doesnt_exist.md', json={'wrapping_key': format_public_key(generate_key_pair()[1])})
    assert response.status_code == 404


def test_posting_file_not_in_config():
    server = get_test_client("examples/config/single_file_single_policy.yml")
    response = server.post('/fetch/license', json={'wrapping_key': format_public_key(generate_key_pair()[1])})
    assert response.status_code == 404
...
def test_posting_dir_in_config():
    ...
    response = server.post('/fetch/examples', json={'wrapping_key': format_public_key(generate_key_pair()[1])})
    assert response.status_code == 200
